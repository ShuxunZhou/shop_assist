...
async def make_graph():
    ...
    def call_list_tables(state: SQLState):
        ...
        # 报错： MultiServerMCPClient 不能异步调用节点。
        # 调用工具：
        ...
        return {"messages": [tool_call_message]}  # 返回状态}
    ...
    def check_query(state: SQLState):
        # 第六个节点：检查 SQL 语法，并且要执行一下这个SQL语句，检查是否有错误
        system_message = {
            "role": "system",
    ...
    workflow.add_edge("get_schema", "generator_query")
